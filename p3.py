...
var MX=86, MY=26;
var k = Math.sqrt(Math.max(W*H,1)/N.length)*1.0, temp = W/6;
...
function pintar(){
  ...
  orden.forEach(function(i){
    ...
    var w=cx.measureText(n.l).width;
    var x=Math.max(5,Math.min(W-w-5, n.x-w/2)), y=n.y-n.r-7;
    var c={x0:x-6,y0:y-15,x1:x+w+6,y1:y+7};
    var choca=false;
    for(var j=0;j<cajas.length;j++){ var o=cajas[j];
      if(!(c.x1<o.x0||c.x0>o.x1||c.y1<o.y0||c.y0>o.y1)){ choca=true; break; } }
    if(choca && !act) return;
    cajas.push(c); etiqueta(n,i,act,x,y);
  });
}
